...
def oi_strength_from_chg(oi_chg: float, *, buildup: bool = True) -> str:
    """OI 强度分级。平仓/去杠杆（ΔOI≤0 或非增仓）即使 |ΔOI| 大也禁止标强/极强。"""
    if not buildup or oi_chg is None or oi_chg <= 0:
        return "普通"
    if oi_chg >= 10:
        return "极强"
    if oi_chg >= 3:
    ...
    if oi_now > 0:
        ...
        if conc >= CONCENTRATION_PCT:
            ...
            if aligned and side != "unknown":
                ...
                # 集中度只是主力代理；OI强度仍看增仓窗，减仓禁止标强/极强
                oi_ref = accel_profile.get("oi_chg_15m")
                if oi_ref is None:
                    oi_ref = accel_profile.get("oi_chg_5m")
                conc_buildup = oi_ref is not None and oi_ref > 0
                conc_strength = oi_strength_from_chg(float(oi_ref or 0.0), buildup=conc_buildup)
                conc_note = "集中度是主力代理，不是开仓许可"
                if not conc_buildup:
                    conc_note += "；当前短线OI未增仓/减仓，强度仅普通"
                events.append(
                    make_event(
                        "OI_CONCENTRATION",
                        symbol,
